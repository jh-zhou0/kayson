package cn.zjh.kayson.framework.apilog.core.filter;
...
import cn.zjh.kayson.framework.apilog.core.service.ApiAccessLog;
...
public class ApiAccessLogFilter extends ApiRequestFilter {
    ...
    private void createApiAccessLog(HttpServletRequest request, LocalDateTime beginTime, 
        ...
        try {
            buildApiAccessLogDTO(accessLog, request, beginTime, queryString, requestBody, ex);
            apiAccessLogFrameworkService.createApiAccessLog(accessLog);
        } catch (Throwable th) {
            log.error("[createApiAccessLog][url({}) log({}) 发生异常]", 
                    request.getRequestURI(), JsonUtils.toJsonString(accessLog), th);
        }
    }

    private void buildApiAccessLogDTO(ApiAccessLog accessLog, HttpServletRequest request, 
                                      LocalDateTime beginTime, Map<String, String> queryString, String requestBody, Exception ex) {
        // 处理用户信息
        ...
        Map<String, Object> requestParams = mapBuilder.put("query", queryString).put("body", requestBody).build();
        ...
        accessLog.setDuration((int) LocalDateTimeUtil.between(beginTime, accessLog.getEndTime(), ChronoUnit.MILLIS));
    }
}
