package cn.zjh.kayson.framework.operatelog.core.aop;
...
/**
 * 拦截使用 @Operation or @OperateLog 注解，如果满足条件，则生成操作日志。
 * 满足如下任一条件，则会进行记录：
 * 1. 使用 @Operation + 非 @GetMapping
 * 2. 不使用 @Operation + 使用 @OperateLog 注解
 * <p>
...
public class OperateLogAspect {
    ...
    public Object around(ProceedingJoinPoint joinPoint, Operation operation) throws Throwable {
        // 可能也添加了 @OperateLog，综合考虑
        cn.zjh.kayson.framework.operatelog.core.annotations.OperateLog operateLog = getMethodAnnotation(joinPoint, 
                cn.zjh.kayson.framework.operatelog.core.annotations.OperateLog.class);
    ...
    public Object around(ProceedingJoinPoint joinPoint, 
                         cn.zjh.kayson.framework.operatelog.core.annotations.OperateLog operateLog) throws Throwable {
        return around0(joinPoint, operateLog, null);
    }

    private Object around0(ProceedingJoinPoint joinPoint, 
                           cn.zjh.kayson.framework.operatelog.core.annotations.OperateLog operateLog, 
                           Operation operation) throws Throwable {
        // 目前，只有管理员，才记录操作日志！所以非管理员，直接调用，不进行记录
        Integer userType = WebFrameworkUtils.getLoginUserType();
        if (ObjectUtil.notEqual(userType, UserTypeEnum.ADMIN.getValue())) {
    ...
    private void fillModuleFields(OperateLog operateLogObj, 
        ...
        if (Objects.isNull(operateLogObj.getType())) {
    ...
    private void fillMethodFields(OperateLog operateLogObj, 
                                  ProceedingJoinPoint joinPoint, 
                                  cn.zjh.kayson.framework.operatelog.core.annotations.OperateLog operateLog, 
                                  LocalDateTime startTime, Object result, Throwable exception) {
    ...
    private String obtainMethodArgs(ProceedingJoinPoint joinPoint) {
        ...
        HashMap<String, Object> map = Maps.newHashMapWithExpectedSize(argNames.length);
        for (int i = 0; i < argNames.length; i++) {
            String argName = argNames[i];
            Object argValue = argValues[i];
            // 被忽略时，标记为 ignore 字符串，避免和 null 混在一起
            map.put(argName, isIgnoreArgs(argValue) ? "[ignore]" : argValue);
        }
    ...
    private boolean isIgnoreArgs(Object object) {
        ...
        if (Collection.class.isAssignableFrom(clazz)) {
            return ((Collection<?>) object).stream().anyMatch((Predicate<Object>) this::isIgnoreArgs);
        }
        // 处理Map
    ...
    private boolean isLogEnable(ProceedingJoinPoint joinPoint,
                                cn.zjh.kayson.framework.operatelog.core.annotations.OperateLog operateLog) {
        // 有 @OperateLog 注解的情况下
        if (operateLog != null) {
    ...
    private static RequestMethod obtainFirstMatchRequestMethod(RequestMethod[] requestMethods) {
        ...
        // 然后，匹配次优的 GET
        result = Arrays.stream(requestMethods).filter(requestMethod -> 
                requestMethod == RequestMethod.GET).findFirst().orElse(null);
        if (result != null) {
            return result;
        }
        // 兜底，获得第一个
    ...
    private static RequestMethod[] obtainRequestMethod(ProceedingJoinPoint joinPoint) {
        ...
        RequestMapping requestMapping = AnnotationUtils.getAnnotation(methodSignature.getMethod(), RequestMapping.class);
    ...
    private static <T extends Annotation> T getClassAnnotation(ProceedingJoinPoint joinPoint, Class<T> annotationClass) {
        MethodSignature methodSignature = (MethodSignature) joinPoint.getSignature();
        return methodSignature.getMethod().getDeclaringClass().getAnnotation(annotationClass);
    }
