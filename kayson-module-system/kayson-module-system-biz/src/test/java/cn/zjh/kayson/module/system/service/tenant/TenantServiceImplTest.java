...
import cn.zjh.kayson.module.system.controller.admin.tenant.vo.tenant.TenantExportReqVO;
import cn.zjh.kayson.module.system.controller.admin.tenant.vo.tenant.TenantPageReqVO;
...
import cn.zjh.kayson.module.system.dal.mysql.tenant.TenantMapper;
...
import cn.zjh.kayson.module.system.service.tenant.handler.TenantInfoHandler;
import cn.zjh.kayson.module.system.service.tenant.handler.TenantMenuHandler;
...
import static cn.zjh.kayson.module.system.dal.dataobject.tenant.TenantDO.PACKAGE_ID_SYSTEM;
...
public class TenantServiceImplTest extends BaseDbUnitTest {
    ...
    void testCreateTenant() {
        ...
        when(userService.createUser(argThat(user -> {
            assertEquals("kayson", user.getUsername());
            assertEquals("kayson", user.getPassword());
            assertEquals("空山木巷", user.getNickname());
            assertEquals("18988998899", user.getMobile());
            return true;
        }))).thenReturn(300L);
        // 准备参数
        TenantCreateReqVO reqVO = randomPojo(TenantCreateReqVO.class, o -> {
            o.setContactName("空山木巷");
            o.setContactMobile("18988998899");
            o.setPackageId(100L);
            o.setStatus(randomCommonStatus());
            o.setDomain("https://www.baidu.com");
            o.setUsername("kayson");
            o.setPassword("kayson");
        });
    ...
    public void testUpdateTenant_success() {
        ...
        when(tenantPackageService.validateTenantPackage(eq(reqVO.getPackageId()))).thenReturn(tenantPackage);
        // mock 所有角色
    ...
    public void testGetTenantPage() {
        ...
        // 测试 createTime 不匹配
        tenantMapper.insert(cloneIgnoreId(dbTenant, o -> o.setCreateTime(buildTime(2023, 12, 12))));
        // 准备参数
        TenantPageReqVO reqVO = new TenantPageReqVO();
        reqVO.setName("kayson");
        reqVO.setContactName("空山木巷");
        reqVO.setContactMobile("189");
        reqVO.setStatus(CommonStatusEnum.ENABLE.getStatus());
        reqVO.setCreateTime(buildBetweenTime(2023, 6, 1, 2023, 6, 15));
    ...
    public void testGetTenantByName() {
        // mock 数据
        TenantDO dbTenant = randomPojo(TenantDO.class, o -> o.setName("kayson"));
        tenantMapper.insert(dbTenant);// @Sql: 先插入出一条存在的数据

        // 调用
        TenantDO result = tenantService.getTenantByName("kayson");
        // 校验存在
        assertPojoEquals(result, dbTenant);
    }
    ...
    @Test
    public void testValidateTenant_notExists() {
        assertServiceException(() -> tenantService.validateTenant(randomLong()), TENANT_NOT_EXISTS);
    }

    @Test
    public void testValidateTenant_disable() {
        // mock 数据
    ...
    @Test
    public void testValidateTenant_expired() {
        // mock 数据
    ...
    @Test
    public void testValidateTenant_success() {
        // mock 数据
        TenantDO tenant = randomPojo(TenantDO.class, o -> o.setId(1L).setStatus(CommonStatusEnum.ENABLE.getStatus())
