...
import cn.zjh.kayson.module.system.convert.tenant.TenantConvert;
...
public class TenantServiceImpl implements TenantService {
    ...
    @Autowired(required = false) // 由于 kayson.tenant.enable 配置项，可以关闭多租户的功能，所以这里只能不强制注入
    ...
    public Long createTenant(TenantCreateReqVO createReqVO) {
        // 校验租户名
        validTenantNameDuplicate(null, createReqVO.getName());
        // 校验套餐
        TenantPackageDO tenantPackage = tenantPackageService.validateTenantPackage(createReqVO.getPackageId());
    ...
    private Long createUser(Long roleId, TenantCreateReqVO createReqVO) {
        // 创建用户
        UserCreateReqVO reqVO = TenantConvert.INSTANCE.convert02(createReqVO);
    ...
    private void validTenantNameDuplicate(Long id, String name) {
        TenantDO tenant = getTenantByName(name);
        if (tenant == null) {
            return;
        }
    ...
    public void updateTenant(TenantUpdateReqVO updateReqVO) {
        ...
        // 校验租户名
        validTenantNameDuplicate(tenant.getId(), updateReqVO.getName());
        // 校验套餐
        TenantPackageDO tenantPackage = tenantPackageService.validateTenantPackage(updateReqVO.getPackageId());
    ...
    public void updateTenantRoleMenu(Long tenantId, Set<Long> menuIds) {
        TenantUtils.execute(tenantId, () -> {
            // 获得所有角色
            List<RoleDO> roleList = roleService.getRoleListByStatus(null);
            ...
            roleList.forEach(role -> {
                ...
                // 如果是其他角色，则去掉超过套餐的权限
                Set<Long> roleMenuIds = CollUtil.intersectionDistinct(permissionService.getRoleMenuIds(role.getId()), menuIds);
    ...
    public List<Long> getTenantIdList() {
        ...
        return CollectionUtils.convertList(tenantList, TenantDO::getId);
    }
